function orderStudentGrade(students) {
  ...
  const result ={};
  for(const i=0;i<students.length-1;i++){
    const key =student.class;
    if(!acc[key]){
     result[key]=[];
     result[key].push(student);
    // for(const j=0;j<students.length-i-1;j++){
...
Object.keys(result).forEach((cls) => {
    ...
    students.sort(a,b) => {
      const a=a.classPerformance+a.defense+a.finalExam;
      const b=b.classPerformance+a.defense+a.finalExam;
